...
class Configuration:
    ...
    def check():
        ...
        try:
            ...
            raise OSError(
                "The standard C library is not defined, please see Configuration.setStandardCLibrary(<file>)"
            )
        ...
        try:
            ...
            raise OSError(
                "The Borg MOEA C library is not defined, please see Configuration.setBorgLibrary(<file>)"
            )
    ...
    def setStandardCLibrary(path=None):
        ...
        if path:
            ...
            try:
                ...
                return
        elif sys.platform == "win32" and cdll.msvcrt:
    ...
    def setBorgLibrary(path=None):
        ...
        if path:
            ...
            try:
                ...
                Configuration.libborg = CDLL(os.path.join(_dir, "libborg.so"))
    ...
    def seed(value=None):
        ...
        if value:
            ...
            Configuration.libborg.BORG_Random_seed(
                c_ulong(os.getpid() * long(time.time()))
            )
    ...
    def startMPI():
        ...
        try:
            ...
            try:
                ...
                try:
                    Configuration.setBorgLibrary(os.path.join(_dir, "libborgms.so"))
...
class Borg:
    ...
    def __init__(
        ...
        self.reference = c_void_p(
            Configuration.libborg.BORG_Problem_create(
    ...
    def _setEpsilon(self, index, value):
        ...
        Configuration.libborg.BORG_Problem_set_epsilon(
            self.reference, index, c_double(value)
        )

    def _setBounds(self, index, lowerBound, upperBound):
        """Sets the lower and upper decision variable bounds at the given index."""
        Configuration.libborg.BORG_Problem_set_bounds(
            self.reference, index, c_double(lowerBound), c_double(upperBound)
        )
    ...
    def solveMPI(
        ...
        if not Configuration.startedMPI:
            raise RuntimeError(
                "MPI is not started; call Configuration.startMPI() first"
            )
        ...
        if islands > 1:
            try:
                ...
                raise RuntimeError(
                    "The loaded Borg MOEA C library does not support multi-master"
                )
        ...
        if maxEvaluations:
            Configuration.libborg.BORG_Algorithm_ms_max_evaluations(
                c_int(maxEvaluations)
            )

        if initialization and islands > 1:
            Configuration.libborg.BORG_Algorithm_ms_initialization(
                c_int(initialization)
            )
        ...
        if allEvaluations:
            Configuration.libborg.BORG_Algorithm_output_evaluations(
                c_char_p(allEvaluations)
            )

        result = c_void_p(Configuration.libborg.BORG_Algorithm_ms_run(self.reference))

        return Result(result, self) if result.value else None
    ...
    def solve(self, settings={}):
        ...
        pm = c_void_p(
            Configuration.libborg.BORG_Operator_create(
                "PM", 1, 1, 2, Configuration.libborg.BORG_Operator_PM
            )
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            pm, 0, c_double(settings.get("pm.rate", 1.0 / self.numberOfVariables))
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            pm, 1, c_double(settings.get("pm.distributionIndex", 20.0))
        )

        sbx = c_void_p(
            Configuration.libborg.BORG_Operator_create(
                "SBX", 2, 2, 2, Configuration.libborg.BORG_Operator_SBX
            )
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            sbx, 0, c_double(settings.get("sbx.rate", 1.0))
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            sbx, 1, c_double(settings.get("sbx.distributionIndex", 15.0))
        )
        Configuration.libborg.BORG_Operator_set_mutation(sbx, pm)

        de = c_void_p(
            Configuration.libborg.BORG_Operator_create(
                "DE", 4, 1, 2, Configuration.libborg.BORG_Operator_DE
            )
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            de, 0, c_double(settings.get("de.crossoverRate", 0.1))
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            de, 1, c_double(settings.get("de.stepSize", 0.5))
        )
        Configuration.libborg.BORG_Operator_set_mutation(de, pm)

        um = c_void_p(
            Configuration.libborg.BORG_Operator_create(
                "UM", 1, 1, 1, Configuration.libborg.BORG_Operator_UM
            )
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            um, 0, c_double(settings.get("um.rate", 1.0 / self.numberOfVariables))
        )

        spx = c_void_p(
            Configuration.libborg.BORG_Operator_create(
                "SPX",
                c_int(settings.get("spx.parents", 10)),
                c_int(settings.get("spx.offspring", 2)),
                1,
                Configuration.libborg.BORG_Operator_SPX,
            )
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            spx, 0, c_double(settings.get("spx.epsilon", 3.0))
        )

        pcx = c_void_p(
            Configuration.libborg.BORG_Operator_create(
                "PCX",
                c_int(settings.get("pcx.parents", 10)),
                c_int(settings.get("pcx.offspring", 2)),
                2,
                Configuration.libborg.BORG_Operator_PCX,
            )
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            pcx, 0, c_double(settings.get("pcx.eta", 0.1))
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            pcx, 1, c_double(settings.get("pcx.zeta", 0.1))
        )

        undx = c_void_p(
            Configuration.libborg.BORG_Operator_create(
                "UNDX",
                c_int(settings.get("undx.parents", 10)),
                c_int(settings.get("undx.offspring", 2)),
                2,
                Configuration.libborg.BORG_Operator_UNDX,
            )
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            undx, 0, c_double(settings.get("undx.zeta", 0.5))
        )
        Configuration.libborg.BORG_Operator_set_parameter(
            undx, 1, c_double(settings.get("undx.eta", 0.35))
        )

        algorithm = c_void_p(
            Configuration.libborg.BORG_Algorithm_create(self.reference, 6)
        )
        ...
        Configuration.libborg.BORG_Algorithm_set_initial_population_size(
            algorithm, c_int(settings.get("initialPopulationSize", 100))
        )
        Configuration.libborg.BORG_Algorithm_set_minimum_population_size(
            algorithm, c_int(settings.get("minimumPopulationSize", 100))
        )
        Configuration.libborg.BORG_Algorithm_set_maximum_population_size(
            algorithm, c_int(settings.get("maximumPopulationSize", 10000))
        )
        Configuration.libborg.BORG_Algorithm_set_population_ratio(
            algorithm, c_double(1.0 / settings.get("injectionRate", 0.25))
        )
        Configuration.libborg.BORG_Algorithm_set_selection_ratio(
            algorithm, c_double(settings.get("selectionRatio", 0.02))
        )
        Configuration.libborg.BORG_Algorithm_set_restart_mode(
            algorithm, c_int(settings.get("restartMode", RestartMode.DEFAULT))
        )
        Configuration.libborg.BORG_Algorithm_set_max_mutation_index(
            algorithm, c_int(settings.get("maxMutationIndex", 10))
        )
        Configuration.libborg.BORG_Algorithm_set_probability_mode(
            algorithm, c_int(settings.get("probabilityMode", ProbabilityMode.DEFAULT))
        )
        ...
        while Configuration.libborg.BORG_Algorithm_get_nfe(algorithm) < maxEvaluations:
            ...
            if (
                ...
                and currentEvaluations - lastSnapshot >= frequency
            ):
                entry = {}
                ...
                entry["UNDX"] = Configuration.libborg.BORG_Operator_get_probability(
                    undx
                )
                entry["UM"] = Configuration.libborg.BORG_Operator_get_probability(um)
                entry["Improvements"] = (
                    Configuration.libborg.BORG_Algorithm_get_number_improvements(
                        algorithm
                    )
                )
                entry["Restarts"] = (
                    Configuration.libborg.BORG_Algorithm_get_number_restarts(algorithm)
                )
                entry["PopulationSize"] = (
                    Configuration.libborg.BORG_Algorithm_get_population_size(algorithm)
                )
                entry["ArchiveSize"] = (
                    Configuration.libborg.BORG_Algorithm_get_archive_size(algorithm)
                )

                if (
                    settings.get("restartMode", RestartMode.DEFAULT)
                    == RestartMode.ADAPTIVE
                ):
                    entry["MutationIndex"] = (
                        Configuration.libborg.BORG_Algorithm_get_mutation_index(
                            algorithm
                        )
                    )
                if fp is None:
                    ...
                    archive = Result(
                        c_void_p(
                            Configuration.libborg.BORG_Algorithm_get_result(algorithm)
                        ),
                        self,
                        statistics,
                    )
                    if runtimeformat == "optimizedv":
                        row = [
                            "{0}".format(entry[dynamic]) for dynamic in dynamics_header
                        ]
                        fp.write("//")
                        fp.write(",".join(row))
                        fp.write("\n")
                        ...
                        if "data_header" in settings and data_header_written is False:
                            ...
                            data_header = [
                                "_".join(x.split(" ")) for x in settings["data_header"]
                            ]
...
class Solution:
    ...
    def violatesConstraints(self):
        ...
        return (
            Configuration.libborg.BORG_Solution_violates_constraints(self.reference)
            != 0
        )
...
class Result:
    ...
    def get(self, index):
        ...
        return Solution(
            c_void_p(Configuration.libborg.BORG_Archive_get(self.reference, index)),
            self.problem,
        )
        ...
        try:
            ...
            if objectives:
                if len(objectives) != numberOfObjectives:
                    raise ValueError(
                        "Incorrect number of objectives returned by function"
                    )
            ...
            if constraints:
                if len(constraints) != numberOfConstraints:
                    raise ValueError(
                        "Incorrect number of constraints returned by function"
                    )
