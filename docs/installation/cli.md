...
If you already have `conda` available, skip to [Step 2](#step-2-get-the-course-repository).
...
1. Download the course ZIP from Brightspace or GitHub.
2. Unzip and navigate in:
```bash
cd path/to/epa141a
```
:::
...
JUSTICE is maintained in a separate repository. Clone it into the repo root as `JUSTICE-main/`:
